...
class SavedPlaylistManager {
    ...
    normalizeSong(s) {
        ...
        return {
            ...
            name: s.name || s.songname, // Keep name for compatibility
...
class UIManager {
    ...
    initWaveform() {
        this.waveform = new Waveform('waveform-canvas', 'waveform-container');

        // Hover time display
        const hoverTimeEl = document.getElementById('waveform-hover-time');

        this.waveform.onSeek = (percent) => {
            if (window.player && window.player.audio.duration) {
                window.player.seek(window.player.audio.duration * percent);
            }
        };

        // Update hover time
        const container = document.getElementById('waveform-container');
        if (container && hoverTimeEl) {
            container.addEventListener('mousemove', (e) => {
                if (!window.player || !window.player.audio.duration) return;

                const rect = container.getBoundingClientRect();
                const x = e.clientX - rect.left;
                const percent = Math.max(0, Math.min(1, x / rect.width));
                const time = window.player.audio.duration * percent;
    ...
    showPlaylistDetail(playlist) {
        ...
        // Reset background gradient first
        ...
        if (cover && detailHeader) {
            extractDominantColor(cover).then(color => {
                // Use a soft gradient: 30% opacity at top -> transparent at bottom
                detailHeader.style.background = `linear-gradient(180deg, ${color.hex}4d 0%, transparent 100%)`;
            }).catch(e => {
                console.warn('Failed to extract playlist color', e);
            });
        }
    }
    ...
    updateProgress(curr, total) {
        ...
        if (this.waveform) {
            this.waveform.updateProgress(curr, total);
        }
    }

    updateSongInfo(song) {
        // 更新底部栏信息
        this.els.barTitle.textContent = song.title || song.name;
    ...
    renderPlaylist(queue, currentIndex, containerId = 'playlist-list', isHistory = false, isDetailPage = false) {
        ...
        queue.forEach((song, i) => {
            ...
            div.innerHTML = `
                ...
                ${isDetailPage ? `
                <div class="item-actions">
                    <button class="action-btn add-btn" title="下一首播放">
            ...
            div.addEventListener('click', (e) => {
                ...
                if (addBtn) {
                    e.stopPropagation();
                    // Use addNext for "Add" button typically or addToQueue?
                    // Search results use addNext when clicking "+". Let's stick to that or addToQueue.
                    // User said "Add to list", usually implies add to queue.
                    // But let's check search behavior - user often likes "Add to next".
                    // Let's use addNext as per search pattern, or simple addToQueue.
                    // Search uses: window.player.addNext(songData);
                    window.player.addNext(song);
                    // ui.notify('已添加到下一首播放', 'success'); // addNext usually notifies?
                    // Let's assume addNext handles notification or we adding explicit one?
                    // window.player.addNext usually handles logic.
                    return;
                }
...
class PlayerManager {
    ...
    async playSong(song) {
        ...
        try {
            ...
            if (this.loadingMid !== song.mid) {
                console.log(`Playback cancelled for ${song.title || song.name} (race condition)`);
                return;
            }

            if (!result.url) {
                console.warn('无法获取播放链接');
                this.ui.notify(`无法播放: ${song.title || song.name} (可能是无版权或VIP歌曲)`, 'error');
            ...
            // Load Waveform
            if (this.ui.waveform) {
                this.ui.waveform.load(result.url, song.mid);
            }
            ...
            document.title = `${song.title || song.name} - ${artistName}`;
    ...
    addToQueue(song, silent = false) {
        ...
        this.ui.renderPlaylist(this.queue, this.currentIndex);
        if (!silent) {
            this.ui.notify(`已添加: ${song.title || song.name}`);
        }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const updateVolSlider = () => {
        ...
        volSlider.style.background = `linear-gradient(to right, var(--accent) ${percent}%, rgba(255, 255, 255, 0.1) ${percent}%)`;
    };

    // Init volume slider
    if (volSlider) {
        volSlider.value = player.audio.volume;
        updateVolSlider();
    ...
    document.addEventListener('keydown', (e) => {
        ...
        switch (e.code) {
            ...
            case 'ArrowUp':
                e.preventDefault();
                player.audio.volume = Math.min(1, player.audio.volume + 0.1);
                if (typeof volSlider !== 'undefined' && volSlider) {
                    volSlider.value = player.audio.volume;
                    updateVolSlider();
                }
                break;
    ...
    /* Legacy progress bar seek removed
    ...
    */
    ...
    const selectionModal = document.getElementById('import-selection-modal');
    ...
    document.getElementById('selection-modal-close').onclick = () => {
    ...
    document.getElementById('fetch-user-playlists').onclick = async () => {
        ...
        // Use a temporary notification or button state instead of clearing a container we don't see yet
        const fetchBtn = document.getElementById('fetch-user-playlists');
        ...
        try {
            ...
            // Render list into the selection modal
            ...
            ui.notify(`获取失败: ${e.message}`, 'error');
            fetchBtn.textContent = originalBtnText;
            fetchBtn.disabled = false;
        }
    };
    ...
    document.getElementById('import-selected-confirm').onclick = async () => {
        ...
        if (selectedPlaylists.length > 0) {
            ...
            if (success) {
                selectionModal.classList.remove('active');
                // Reset Input
                document.getElementById('import-uid-input').value = '';
            }
        } else {
    ...
    // Import All
    document.getElementById('import-all-btn').onclick = async () => {
        if (window.currentImportList && window.currentImportList.length > 0) {
            const success = await savedPlaylistManager.importPlaylists(window.currentImportList);
            if (success) {
                selectionModal.classList.remove('active');
                // Reset Input
                document.getElementById('import-uid-input').value = '';
